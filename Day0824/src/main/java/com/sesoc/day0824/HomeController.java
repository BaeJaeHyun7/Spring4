...
public class HomeController {
	...
	//파라미터의 배열의 이름은 test1에서 data이름과 동일해야 한다.
	//ajax통신은 @ResponseBody를 붙여줘야 한다.
	@ResponseBody
	...
	public void sendArray(String[] arr) {
		...
		if (arr != null) {
			for(String s : arr)
				logger.info("페이지로부터 전달 받은 데이터: {}", s);
		}else
			logger.info("페이지로부터 전달 받은 데이터: {}", arr);
	}
	...
	public void sendVO(UserVO user) {
		...
		logger.info("페이지로부터 전달 받은 데이터: {}", user);
	...
	public void sendList(@RequestBody ArrayList<String> list) {
		...
		logger.info("페이지로부터 전달 받은 데이터: {}", list);
	...
	public void sendMap(@RequestBody HashMap<String, Object> map) {
		...
		logger.info("페이지로부터 전달 받은 데이터: {}", map);
